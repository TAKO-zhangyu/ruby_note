...
# やりたいことは、User id=1　が　Message id=3 にいいねをする　仕組みを作りたい。
...
# 1人のユーザーから見れば自分の投稿は複数あるが、ツイートから見れば投稿主は一人のみ。なので、それぞれの関係は１つしかなく、書き換えるとMessage_id=1 の中にはUser_id=1の情報しかなく、1つのカラムにそれ以外の情報を入れることはない。
...
# これを解決するために、多対多のときは中間テーブルを作り、この場合はUser_idとMessager_idのカラムを作る。このテーブルの中で、User_id=1 + Message_id=1, User_id=1 + Message_id=2 User_id=2 + Message_id=5のような組み合わせを作る。具体的な処理は以下から。 
